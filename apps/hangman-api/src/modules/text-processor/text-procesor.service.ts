...
export class TextProcessorService implements OnModuleDestroy {
  ...
  constructor(
    @Inject(MARKDOWN_PROCESSOR)
    private readonly markdownProcessor: MarkdownProcessor,
  ) {
    this.processMarkdown = promisify(this.markdownProcessor.process).bind(
      markdownProcessor,
    );
  }
  ...
  async onModuleDestroy() {
    this.markdownProcessor.shutdown(() =>
      console.log('MarkdownProcessor finished'),
    );
  }
}
